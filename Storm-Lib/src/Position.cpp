...
namespace Storm
{
    ...
    void Position::Initialize()
    {
        ...
        STORM_ASSERT(Cache.KingSquare[COLOR_WHITE] != SQUARE_INVALID && Cache.KingSquare[COLOR_BLACK] != SQUARE_INVALID,
          "Couldn't find king");
    ...
    bool Position::GivesCheck(Move move) const
    {
        ...
        // Discovered check
        if (GetBlockersForKing(OtherColor(ColorToMove)) & fromSquare &&
            !IsAligned(fromSquare, toSquare, GetKingSquare(OtherColor(ColorToMove))))
            return true;

        if (GetMoveType(move) == PROMOTION)
            return GetAttacksDynamic(GetPromotionPiece(move), toSquare, GetPieces() ^ fromSquare) &
                   GetKingSquare(OtherColor(ColorToMove));
        ...
        if (movingPiece == PIECE_PAWN && toSquare == EnpassantSquare)
        {
            ...
            return ((GetAttacks<PIECE_BISHOP>(GetKingSquare(OtherColor(ColorToMove)), blockers) &
                      GetPieces(ColorToMove, PIECE_BISHOP, PIECE_QUEEN)) |
                    (GetAttacks<PIECE_ROOK>(GetKingSquare(OtherColor(ColorToMove)), blockers) &
                      GetPieces(ColorToMove, PIECE_ROOK, PIECE_QUEEN)));
        }
        ...
        if (GetMoveType(move) == CASTLE)
        {
            ...
            if (toFile == FILE_G)
            {
                ...
                return GetAttacks<PIECE_ROOK>(rookTo, (GetPieces() ^ rookFrom ^ fromSquare) | toSquare | rookTo) &
                       GetKingSquare(OtherColor(ColorToMove));
            }
            else
            {
    ...
    bool Position::IsLegal(Move move) const
    {
        ...
        if (toSquare == EnpassantSquare && movingPiece == PIECE_PAWN)
        {
            ...
            return !(GetAttacks<PIECE_ROOK>(kingSquare, occupied) &
                     GetPieces(OtherColor(ColorToMove), PIECE_QUEEN, PIECE_ROOK)) &&
                   !(GetAttacks<PIECE_BISHOP>(kingSquare, occupied) &
                     GetPieces(OtherColor(ColorToMove), PIECE_QUEEN, PIECE_BISHOP));
        }
        ...
        return !(GetBlockersForKing(ColorToMove) & fromSquare) || IsAligned(fromSquare, toSquare, kingSquare);
    }
    ...
    BitBoard Position::GetSliderBlockers(BitBoard sliders, SquareIndex toSquare, BitBoard* pinners) const
    {
        ...
        *pinners = ZERO_BB;
        BitBoard snipers = ((GetAttacks<PIECE_ROOK>(toSquare, ZERO_BB) & GetPieces(PIECE_QUEEN, PIECE_ROOK)) |
                             (GetAttacks<PIECE_BISHOP>(toSquare, ZERO_BB) & GetPieces(PIECE_QUEEN, PIECE_BISHOP))) &
                           sliders;
    ...
    {
        Cache.BlockersForKing[COLOR_WHITE] =
          GetSliderBlockers(GetPieces(COLOR_BLACK), GetKingSquare(COLOR_WHITE), &Cache.Pinners[COLOR_BLACK]);
        Cache.BlockersForKing[COLOR_BLACK] =
          GetSliderBlockers(GetPieces(COLOR_WHITE), GetKingSquare(COLOR_BLACK), &Cache.Pinners[COLOR_WHITE]);
        ...
        Cache.CheckSquares[color][PIECE_QUEEN - PIECE_START] =
          Cache.CheckSquares[color][PIECE_BISHOP - PIECE_START] | Cache.CheckSquares[color][PIECE_ROOK - PIECE_START];
    ...
    std::string GetFENFromPosition(const Position& position)
    {
        ...
        result += ' ';
        if (position.Colors[COLOR_WHITE].CastleKingSide || position.Colors[COLOR_WHITE].CastleQueenSide ||
            position.Colors[COLOR_BLACK].CastleKingSide || position.Colors[COLOR_BLACK].CastleQueenSide)
        {
