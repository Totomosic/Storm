...
namespace Storm::NNUE {
  ...
  class FeatureTransformer {
   ...
   public:
    ...
    static constexpr IndexType InputDimensions = HalfKAv2::Dimensions;
    ...
    static constexpr std::uint32_t get_hash_value() {
      return HalfKAv2::HashValue ^ OutputDimensions;
    }
   ...
   private:
    void update_accumulator(const Position& pos, const Color perspective) const {
      ...
      using IndexList = ValueList<IndexType, HalfKAv2::MaxActiveDimensions>;
      ...
      int gain = HalfKAv2::refresh_cost(pos);
      while (st->Previous && !st->Accumulator.computed[perspective])
      {
        // This governs when a full feature refresh is needed and how many
        // updates are better than just one full refresh.
        if (   HalfKAv2::requires_refresh(st, perspective)
            || (gain -= HalfKAv2::update_cost(st) + 1) < 0)
          break;
        next = st;
        st = st->Previous;
      }
      ...
      if (st->Accumulator.computed[perspective])
      {
        ...
        HalfKAv2::append_changed_indices(
      ...
      else
      {
        ...
        auto& accumulator = pos.GetState()->Accumulator;
        accumulator.computed[perspective] = true;
        IndexList active;
        HalfKAv2::append_active_indices(pos, perspective, active);
