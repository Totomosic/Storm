...
namespace Storm::NNUE {
  ...
  ValueType EvaluateNNUE(const Position& pos, bool adjusted) {
    ...
    int delta_npm = abs(pos.GetNonPawnMaterial(COLOR_WHITE) - pos.GetNonPawnMaterial(COLOR_BLACK));
    int entertainment = (adjusted && delta_npm <= GetPieceValueMg(PIECE_BISHOP) - GetPieceValueMg(PIECE_KNIGHT) ? 7 : 0);
  ...
  bool save_eval(const std::optional<std::string>& filename) {
    ...
    {
        if (true /* eval_file_loaded != EvalFileDefaultName */)
        {
