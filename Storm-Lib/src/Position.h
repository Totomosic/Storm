...
namespace Storm
{
    ...
    public:
        ...
        inline StateInfo* GetState() const
        {
            return m_StateInfo;
        }

        // Mainly for SWIG usage
        inline uint64_t GetHashInteger() const
        {
            return Hash.Hash;
        }
        ...
        {
            return Cache.AllPieces;
        }

        inline BitBoard GetPieces(Piece p0) const
        {
            return GetPieces(COLOR_WHITE, p0) | GetPieces(COLOR_BLACK, p0);
        }

        inline BitBoard GetPieces(Piece p0, Piece p1) const
        {
            return GetPieces(p0) | GetPieces(p1);
        }

        inline BitBoard GetPieces(Piece p0, Piece p1, Piece p2) const
        {
            return GetPieces(p0, p1) | GetPieces(p2);
        }

        inline BitBoard GetPieces(Color color) const
        {
            return Cache.ColorPieces[color];
        }

        inline BitBoard GetPieces(Color color, Piece p0) const
        {
            return Colors[color].Pieces[p0];
        }

        inline BitBoard GetPieces(Color color, Piece p0, Piece p1) const
        {
            return GetPieces(color, p0) | GetPieces(color, p1);
        }

        inline BitBoard GetPieces(Color color, Piece p0, Piece p1, Piece p2) const
        {
            return GetPieces(color, p0, p1) | GetPieces(color, p2);
        }

        inline SquareIndex GetKingSquare(Color color) const
        {
            return Cache.KingSquare[color];
        }

        inline ColorPiece GetPieceOnSquare(SquareIndex square) const
        {
            return Cache.PieceOnSquare[square];
        }

        inline bool SquareOccupied(SquareIndex square) const
        {
            return GetPieceOnSquare(square) != COLOR_PIECE_NONE;
        }

        inline BitBoard GetBlockersForKing(Color color) const
        {
            return Cache.BlockersForKing[color];
        }

        inline Color GetColorOnSquare(SquareIndex square) const
        {
            return (GetPieces(COLOR_WHITE) & square) ? COLOR_WHITE : COLOR_BLACK;
        }

        inline bool InCheck() const
        {
            return Cache.CheckedBy != ZERO_BB;
        }

        inline BitBoard GetCheckers() const
        {
            return Cache.CheckedBy;
        };

        inline ValueType GetNonPawnMaterial(Color color) const
        {
            return Cache.NonPawnMaterial[color];
        }

        inline ValueType GetNonPawnMaterial() const
        {
            return GetNonPawnMaterial(COLOR_WHITE) + GetNonPawnMaterial(COLOR_BLACK);
        }

        inline int GetTotalHalfMoves() const
        {
            return 2 * TotalTurns + (ColorToMove == COLOR_BLACK);
        }

        inline Piece GetMovingPiece(Move move) const
        {
            return TypeOf(GetPieceOnSquare(GetFromSquare(move)));
        }

        inline Piece GetCapturedPiece(Move move) const
        {
            return TypeOf(GetPieceOnSquare(GetToSquare(move)));
        }

        inline bool IsCapture(Move move) const
        {
            return GetPieceOnSquare(GetToSquare(move)) != COLOR_PIECE_NONE || IsEnpassant(move);
        }

        inline bool IsEnpassant(Move move) const
        {
            return TypeOf(GetPieceOnSquare(GetFromSquare(move))) == PIECE_PAWN && GetToSquare(move) == EnpassantSquare;
        }
        ...
        inline bool IsNetworkEnabled() const
        {
            return m_UseNetwork;
        }
