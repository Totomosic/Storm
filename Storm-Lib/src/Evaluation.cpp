...
namespace Storm
{
    ...
    void InitPassedPawnMasks()
    {
        for (SquareIndex square = a1; square < SQUARE_MAX; square++)
        {
            ...
            SupportedPawnMasks[COLOR_WHITE][square] =
              Shift<EAST>(sq) | Shift<WEST>(sq) | Shift<SOUTH_EAST>(sq) | Shift<SOUTH_WEST>(sq);
            SupportedPawnMasks[COLOR_BLACK][square] =
              Shift<EAST>(sq) | Shift<WEST>(sq) | Shift<NORTH_EAST>(sq) | Shift<NORTH_WEST>(sq);
        }
    }
    ...
    void ScoreThreats(const Position& position, EvaluationData& data, BitBoard attacks, SquareIndex square,
      ValueType& mg, ValueType& eg)
    {
        BitBoard threatenedPieces =
          attacks & position.GetPieces(OtherColor(C)) & ~position.GetKingSquare(OtherColor(C));
    ...
    void EvaluatePieces(const Position& position, EvaluationResult& result, EvaluationData& data)
    {
        ...
        BitBoard checkSquares =
          ...
          P == PIECE_QUEEN  ? GetAttacks<PIECE_QUEEN>(position.GetKingSquare(OtherColor(C)), position.GetPieces()) :
                              ZERO_BB;
        ...
        while (pieces)
        {
            ...
            BitBoard attacks = P == PIECE_KNIGHT ? GetAttacks<PIECE_KNIGHT>(square) :
                               ...
                               P == PIECE_QUEEN  ? GetAttacks<PIECE_QUEEN>(square, position.GetPieces()) :
                                                   ZERO_BB;
            ...
            if constexpr (P == PIECE_BISHOP || P == PIECE_KNIGHT)
            {
                ...
                if (Shift<Down>(position.GetPieces(C, PIECE_PAWN)) & square &
                    InFrontOrEqual<C>(RelativeRank<C>(RANK_3)))
            ...
            {
                BitBoard pinners;
                if (position.GetSliderBlockers(
                      position.GetPieces(OtherColor(C), PIECE_ROOK, PIECE_BISHOP), square, &pinners))
                {
    ...
    void EvaluateKingSafety(const Position& position, EvaluationResult& result, EvaluationData& data)
    {
        ...
        safeChecks &=
          ~data.AttackedBy[C][PIECE_ALL] | (data.AttackedByTwice[OtherColor(C)] & ~data.AttackedByTwice[C] &
                                             (data.AttackedBy[C][PIECE_KING] | data.AttackedBy[C][PIECE_QUEEN]));
        if (safeChecks)
        {
        ...
        ValueType danger = data.AttackUnits[OtherColor(C)] * data.AttackUnits[OtherColor(C)] *
                           AttackerCountScaling[std::min(data.AttackerCount[OtherColor(C)], MaxAttackerCount - 1)] /
                           MaxAttackerCount;
    ...
    {
        constexpr BitBoard SpaceMask = C == COLOR_WHITE ? (CenterFiles & (RANK_2_BB | RANK_3_BB | RANK_4_BB)) :
        ...
        {
            BitBoard safe =
              SpaceMask & ~position.GetPieces(C, PIECE_PAWN) & ~data.AttackedBy[OtherColor(C)][PIECE_PAWN];
    ...
    {
        data.MobilityArea[C] =
          ~(position.GetPieces(C, PIECE_PAWN) | data.AttackedBy[OtherColor(C)][PIECE_PAWN] | position.GetKingSquare(C));
    }
    ...
    EvaluationResult EvaluateDetailed(const Position& position)
    {
        ...
        result.Initiative = InitiativeBonuses[0] * Popcount(allPawns) +
...
#define FORMAT_TABLE_ROW(Score, Length) \
    (FormatScore(Score[COLOR_WHITE][MIDGAME], Length) + " " + FormatScore(Score[COLOR_WHITE][ENDGAME], Length) + \
      " | " + FormatScore(Score[COLOR_BLACK][MIDGAME], Length) + " " + \
      FormatScore(Score[COLOR_BLACK][ENDGAME], Length) + " | " + \
      FormatScore(Score[COLOR_WHITE][MIDGAME] - Score[COLOR_BLACK][MIDGAME], Length) + " " + \
