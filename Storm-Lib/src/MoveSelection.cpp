...
namespace Storm
{
    ...
    MoveSelector<TYPE>::MoveSelector(const Position& position, SearchStack* stack, Move hashMove, Move counterMove,
      Move killers[2], SearchTables* tables)
        : m_Stage(FIND_TT_MOVE),
          m_Position(position),
          m_HashMove(hashMove),
    ...
    void MoveSelector<TYPE>::GenerateCaptures()
    {
        ...
        while (it != end)
        {
            ...
            {
                it->Value = VALUE_GOOD_CAPTURE + GetPieceValueMg(m_Position.GetCapturedPiece(it->Move)) -
                            GetPieceValueMg(m_Position.GetMovingPiece(it->Move));
            ...
            {
                it->Value = VALUE_BAD_CAPTURE + GetPieceValueMg(m_Position.GetCapturedPiece(it->Move)) -
                            GetPieceValueMg(m_Position.GetMovingPiece(it->Move));
                if (GetMoveType(it->Move) == PROMOTION)
                    it->Value += GetPieceValueMg(GetPromotionPiece(it->Move));
