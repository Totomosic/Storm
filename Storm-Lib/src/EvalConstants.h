...
namespace Storm
{
	...
	constexpr ValueType MateIn(int ply) { return VALUE_MATE - ply; }
	constexpr ValueType MatedIn(int ply) { return -VALUE_MATE + ply; }
	constexpr int GetPliesFromMateScore(ValueType score) { return score < 0 ? score + VALUE_MATE : VALUE_MATE - score; }
	constexpr bool IsMateScore(ValueType score) { return score >= MateIn(MAX_PLY) || score <= MatedIn(MAX_PLY); }
