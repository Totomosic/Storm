...
namespace Storm
{
    ...
    Move UCI::CreateMoveFromString(const Position& position, const std::string& uciString)
    {
        ...
        if (TypeOf(position.GetPieceOnSquare(CreateSquare(startFile, startRank))) == PIECE_KING &&
            startFile == FILE_E && (endFile == FILE_C || endFile == FILE_G))
    ...
    Move PGN::CreateMoveFromString(const Position& position, const std::string& pgnString)
    {
        ...
        if (movingPiece == PIECE_PAWN && !isCapture)
        {
            ...
            if (position.ColorToMove == COLOR_WHITE)
            {
                BitBoard pawns = position.GetPieces(position.ColorToMove, PIECE_PAWN) & FILE_MASKS[pawnFile] &
                                 ~InFrontOrEqual<COLOR_WHITE>(toRank);
            ...
            {
                BitBoard pawns = position.GetPieces(position.ColorToMove, PIECE_PAWN) & FILE_MASKS[pawnFile] &
                                 ~InFrontOrEqual<COLOR_BLACK>(toRank);
                                    ...
                                    else
                                    {
                                        if (move == MOVE_NONE)
                                        {
                                            ...
                                            std::cout << "Side to Move: "
                                                      << ((currentPosition.ColorToMove == COLOR_WHITE) ? 'w' : 'b')
                                                      << std::endl;
