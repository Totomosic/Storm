...
namespace Storm
{
    ...
    inline void SetCounterMoveHistoryPointer(
      int16_t** cmhPtr, SearchStack* stack, CounterMoveHistoryTable* cmhTable, int ply)
    {
    ...
    inline ValueType GetHistoryScore(
      SearchStack* stack, const Position& position, int16_t** cmhPtr, Move move, const SearchTables* tables)
    {
    ...
    inline void AddToHistory(
      const Position& position, SearchStack* stack, int16_t** cmhPtr, SearchTables* tables, Move move, ValueType score)
    {
        SquareIndex from = GetFromSquare(move);
        SquareIndex to = GetToSquare(move);
        int piecePosition = position.GetPieceOnSquare(from) * SQUARE_MAX + to;
